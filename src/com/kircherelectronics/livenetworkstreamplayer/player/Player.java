 ...
 */
package com.kircherelectronics.livenetworkstreamplayer.player;
...
public class Player implements ExoPlayer.Listener,
		...
		DashChunkSource.EventListener, TextRenderer,
		MetadataRenderer<Map<String, Object>>, DebugTextViewHelper.Provider
{
	...
	public interface RendererBuilder
	{
		 ...
		 *            {@link Player#onRenderers} should be invoked once the
		 ...
		 * {@link Player#onRenderers} or {@link Player#onRenderersError}
	...
	public interface InfoListener
	{
		...
		void onLoadStarted(int sourceId, long length, int type, int trigger,
				Format format, int mediaStartTimeMs, int mediaEndTimeMs);

		void onLoadCompleted(int sourceId, long bytesLoaded, int type,
	 ...
	 *            {@link Player} TYPE_* constants. May be null if the track
	 *            names are unknown. An individual element may be null if the
	 *            track names are unknown for the corresponding type.
	 * @param multiTrackSources
	 *            Sources capable of switching between multiple available
	 *            tracks, indexed by {@link Player} TYPE_* constants. May be
	 *            null if there are no types with multiple tracks. An individual
	 *            element may be null if it does not have multiple tracks.
	 * @param renderers
	 *            Renderers indexed by {@link Player} TYPE_* constants. An
			...
			TrackRenderer[] renderers, BandwidthMeter bandwidthMeter)
	{
		...
		for (int rendererIndex = 0; rendererIndex < RENDERER_COUNT; rendererIndex++)
		{
			...
			if (trackNames[rendererIndex] == null)
			{
				...
				int trackCount = multiTrackSources[rendererIndex] != null ? multiTrackSources[rendererIndex]
						.getTrackCount() : 1;
		...
		this.codecCounters = videoRenderer instanceof MediaCodecTrackRenderer ? ((MediaCodecTrackRenderer) videoRenderer).codecCounters
				: renderers[TYPE_AUDIO] instanceof MediaCodecTrackRenderer ? ((MediaCodecTrackRenderer) renderers[TYPE_AUDIO]).codecCounters
						: null;
	...
	public void onMetadata(Map<String, Object> metadata)
	{
		if (id3MetadataListener != null
				&& selectedTracks[TYPE_METADATA] != DISABLED_TRACK)
		{
			...
			Format format, int mediaStartTimeMs, int mediaEndTimeMs)
	{
		if (infoListener != null)
		{
			infoListener.onLoadStarted(sourceId, length, type, trigger, format,
			...
			int mediaEndTimeMs, long elapsedRealtimeMs, long loadDurationMs)
	{
		if (infoListener != null)
		{
			infoListener.onLoadCompleted(sourceId, bytesLoaded, type, trigger,
					format, mediaStartTimeMs, mediaEndTimeMs,
