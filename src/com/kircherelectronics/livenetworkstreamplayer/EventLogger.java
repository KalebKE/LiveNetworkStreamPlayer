 ...
 */
public class EventLogger implements Player.Listener, Player.InfoListener,
		Player.InternalErrorListener
{
			...
			int mediaEndTimeMs, long elapsedRealtimeMs, long loadDurationMs)
	{
		if (VerboseLogUtil.isTagEnabled(TAG))
		{
			long downloadTime = SystemClock.elapsedRealtime()
					- loadStartTimeMs[sourceId];
	...
	public void onSeekRangeChanged(TimeRange seekRange)
	{
		...
		Log.d(TAG, "seekRange [ " + seekRange.type + ", "
				+ seekRangeValuesUs[0] + ", " + seekRangeValuesUs[1] + "]");
	}
